            ...
            while true do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~r~E~w~] Uzmi cash", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~r~E~w~] Uzmi cash", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~r~E~w~] Uzmi cash", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" then
        exports["mythic_notify"]:SendAlert("inform", "Alarmi su oglašeni!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
            ...
            if PlayerData.job.name == "police" and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 4.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Otkljucaj vrata", 0.40)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Zakljucaj vrata", 0.40)
                            end
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    exports["mythic_notify"]:SendAlert("error", "SEF CE SE ZATVORITI ZA 30 SEKUNDI!")
    Citizen.Wait(30000)
    exports["mythic_notify"]:SendAlert("error", "SEF SE ZATVARA!")
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    exports['progressBars']:startUI(2000, "Korištenje id kartice")
    Citizen.Wait(1500)
    ...
    disableinput = false
    Citizen.Wait(1000)
    Process(UTK.hacktime, "Hakiranje...")
    exports["mythic_notify"]:SendAlert("success", "Uspiješno hakiranje!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
    TriggerServerEvent("utk_fh:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    exports["mythic_notify"]:SendAlert("error", "Imaš 2 minute prije nego što se alarm aktivira.")
...
function SecondDoor(data, key)
    ...
    TaskStartScenarioInPlace(ped, "PROP_HUMAN_ATM", 0, true)
    exports['progressBars']:startUI(2000, "Korištenje loše kartice")
    Citizen.Wait(1500)
    ...
    ClearPedTasksImmediately(ped)
    disableinput = false
    Process(2000, "Pristupanje panelu")
    exports["mythic_notify"]:SendAlert("success", "Pristup odobren!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
                ...
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    ESX.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            exports["mythic_notify"]:SendAlert("error", "Nemaš sigurnu id karticu.")
                        end
                    end)
                end
            end
            ...
            for k, v in pairs(UTK.Banks) do
                if not v.onaction then
                    ...
                    if dst <= 5 and not Check[k] then
                        DrawText3D(v.doors.startloc.x, v.doors.startloc.y, v.doors.startloc.z, "[~r~E~w~] Pokreni pljacku", 0.40)
