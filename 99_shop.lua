    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
                    if dist >= 12.0 then
                        pNotify("Izasao si iz trgovine, sve stvari su vracene na policu!", "error", 2500)
                        payAmount = 0
                        Basket = {}
                    end
                end
...
OpenAction = function(action, shelf, text)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            ...
            pNotify("Nemas nista u košari!", 'error', 1500)
        end
    else
                        ...
                        function(data2, menu2)
                            ESX.TriggerServerCallback('99kr-shops:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    ...
                                    pNotify("Nemaš dosta novaca!", 'error', 1500)
                                end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("Stavljeno " .. data.current.value .. " komada " .. data.current["realLabel"] .. " u košaru", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
...
OpenBasket = function()
    if payAmount > 0 and #Basket then
        ...
        table.insert(elements, {label = '<span style="color:red">Isprazni košaru', value = "empty"})
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    pNotify("Sve je vraceno na police.", "error", 2500)
                end
                if data.current.value == "item_menu" then
                    ...
                    ESX.UI.Menu.Open(
                        ...
                        {
                            title    = "Košara - " .. shopItem["label"] .. " - " .. shopItem["amount"] .. "komada",
                            align    = 'center',
                            elements = {
                                {label = shopItem["label"] .. " - $" .. shopItem["price"] * shopItem["amount"]},
                                {label = '<span style="color:red">Vrati stvar na policu</span>', value = "deleteItem"},
                            },
                        },
                        function(data2, menu2)
                            if data2.current["value"] == "deleteItem" then
                                pNotify("Maknuo " .. Basket[index]["amount"] .." ".. Basket[index]["label"] .. " iz košare.", "alert", 2500)
