    ...
    if not info.locked then
        if (os.time() - cooldown) > lastrobbed then
            ...
            if copcount >= mincops then
                ...
                cb("Mora biti "..mincops.." policajaca u gradu.")
            end
        else
            cb(math.floor((cooldown - (os.time() - lastrobbed)) / 60)..":"..math.fmod((cooldown - (os.time() - lastrobbed)), 60).." do druge pljacke.")
        end
    else
        cb("Pljacka je u toku.")
    end
end)
