...
UTK = {
    ...
    alarmblip,
    text = { -- Texts
        loudstart = "[~r~E~w~] Pokreni ~g~GLASNU~w~ pljacku",
        silentstart = "[~r~E~w~] Pokreni ~g~TIHU~w~ pljacku",
        usecard = "[~r~E~w~] Koristi id karticu",
        usethermal = "[~r~E~w~] Koristi bombu",
        usehack = "[~r~E~w~] Hakiraj sigurnosni panel",
        uselockpick = "[~r~E~w~] Koristi Obijac",
        usesearch = "[~r~E~w~] Pretrazi",
        lootcash= "[~r~E~w~] Ukradi novac",
        lootgold = "[~r~E~w~] Ukradi zlato",
        lootdia = "[~r~E~w~] Ukradi dijamante",
        card = "Korištenje id kartice",
        thermal = "Postavljanje termalne bombe",
        burning = "Topljenje...",
        lockpick = "Obijanje lokota",
        using = "Korištenje panela",
        used = "Proces završen.",
        stage = "završeno.",
        search = "Traženje",
        hacking = "Hakiranje",
        melted = "Lokot otopljen.",
        hacked = "Hakiranje uspiješno.",
        unlocked = "Vrata otljucana.",
        nothing = "Ništa.",
        found = "Našao si id karticu.",
        time = "Preostalo: "
    }
}
UTK.searchinfo = {
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            ...
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Nemaš laptop za hakiranje.")
                                            end
                                        end, "laptop_h")
                                    end
                                end
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports["mythic_notify"]:SendAlert("error", "Nemaš id karticu.")
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Nemaš termalnu bombu.")
                                            end
                                        end, "thermal_charge")
                                    end
                                end
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    exports['mythic_notify']:SendAlert("error", "Nemaš obijac.")
                                                end
                                            end, "lockpick")
                                        end
                                    end
...
function UTK:Lockpick()
    ...
    exports['progressBars']:startUI(6000, self.text.lockpick..": Korak 1")
    Citizen.Wait(6500)
    exports['mythic_notify']:SendAlert("success", "Korak 1 "..self.text.stage)
    SetEntityCoords(ped, loc.x, loc.y, loc.z, 1, 0, 0, 1)
    SetEntityHeading(ped, loc.h)
    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 6000, 1, 1, 0, 0, 0)
    exports['progressBars']:startUI(6000, self.text.lockpick..": Korak 2")
    Citizen.Wait(6500)
    exports['mythic_notify']:SendAlert("success", "Korak 2 "..self.text.stage)
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    exports["mythic_notify"]:SendAlert("success", "Otvori My Computer i idi na HackConnect.exe")
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    exports["mythic_notify"]:SendAlert("success", "Pronadi ip adresu...")
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    exports["mythic_notify"]:SendAlert("success", "Potrazi sifru...")
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    exports["mythic_notify"]:SendAlert("success", "Pokreni BruteForce.exe")
                ...
                elseif Hacking and program == 86 then
                    ...
                    elseif UTK.hackmethod == 2 then
                        ...
                        UsingComputer = false
                        Ipfinished = false
                        Process(25000, "Hakiranje sistema...")
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['mythic_notify']:SendAlert("error", "Imaš 90 sekundi dok ti toksicni plin ne pocne izlaziti.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            exports["mythic_notify"]:SendAlert("success", "Ukrao si "..text)
        end)
    else
        exports['mythic_notify']:SendAlert("error", "Imaš 90 sekundi dok ti toksicni plin ne pocne izlaziti.")
        UTK.starttimer = true
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports["mythic_notify"]:SendAlert("error", "Sef ce se zatvoriti za 60 sekundi!")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                exports["mythic_notify"]:SendAlert("error", "Sef ze zatvara!")
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
        if toggle == 1 then
            exports["mythic_notify"]:SendAlert("infrom", "Glavna banka se pljacka!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
        ...
        for k, v in ipairs(PoliceDoors) do
            ...
            if dst <= 50 then
                if v.locked then
                    ...
                    doortext = "[~r~E~w~] Otkljucaj vrata"
                    state = false
                elseif not v.locked then
                    FreezeEntityPosition(v.obj, false)
                    doortext = "[~r~E~w~] Zakljucaj vrata"
                    state = true
                end
                if PlayerData.job.name == "police" then
                    if dst <= 1.5 then
