        ...
        if item >= 1 then
            if not UTK.Banks[bank].onaction == true then
                if (os.time() - UTK.cooldown) > UTK.Banks[bank].lastrobbed then
                    ...
                    TriggerClientEvent("utk_fh:outcome", _source, false, "Ova banka je vec opljackana, pricekaj "..math.floor((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)) / 60)..":"..math.fmod((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)), 60))
                end
            else
                TriggerClientEvent("utk_fh:outcome", _source, false, "Ova banka se trenutno pljacka.")
            end
        else
            TriggerClientEvent("utk_fh:outcome", _source, false, "Nemaš lošu id karticu.")
        end
    else
        TriggerClientEvent("utk_fh:outcome", _source, false, "Nema dovoljno policije u gradu.")
    end
end)
