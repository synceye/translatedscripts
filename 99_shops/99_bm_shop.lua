    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
                    if dist >= 12.0 then
                        pNotify("Izasao si iz crnog marketa, sve stvari su vracene na svoje mjesto!", "error", 2500)
                        payAmount = 0
                        Basket = {}
                    end
                end
...
OpenAction = function(action, shelf, text)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            ...
            pNotify("Nemas nista u svojoj kosarici!", 'error', 1500)
        end
    else
...
CashRegister = function(titel)
        local elements = {
            {label = '<span style="color:lightgreen; border-bottom: 1px solid lightgreen;">Potvrdi</span>', value = "yes"},
            {label = 'Racun: <span style="color:green">$' .. payAmount ..'</span>'},
        }

        for i=1, #Basket do
            local item = Basket[i]
            table.insert(elements, {
                label = '<span style="color:red">*</span> ' .. item["label"] .. ': ' .. item["amount"] .. ' komada',
                value = item["value"],
            })
        end
            ...
            function(data, menu)
                ...
                if data.current.value == "yes" then
                    ...
                    ESX.UI.Menu.Open(
                        ...
                        {
                            title    = "Crni Market - Placanje",
                            align    = 'center',
                            elements = {
                                {label = "Plati kešom", value = "cash"},
                                {label = "Plati karticom", value = "bank"},
                            },
                        },
                        function(data2, menu2)
                            ESX.TriggerServerCallback('99kr-blackmarket:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    TriggerServerEvent('99kr-blackmarket:Cashier', payAmount, Basket, data2.current["value"])
                                    payAmount = 0
                                    Basket = {}
                                    menu2.close()
                                else
                                    pNotify("Nemaš dovoljno novaca!", 'error', 1500)
                                end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("Stavljeno " .. data.current.value .. " komada " .. data.current["realLabel"] .. " u kosaru", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
...
OpenBasket = function()
    if payAmount > 0 and #Basket then
        local elements = {
            {label = 'Racun: <span style="color:green">$' .. payAmount},
        }
        for i=1, #Basket do
            local item = Basket[i]
            table.insert(elements, {
                label = '<span style="color:red">*</span> ' .. item["label"] .. ': ' .. item["amount"] .. ' komada (<span style="color:green">$' .. item["price"] * item["amount"] .. '</span>)',
                value = "item_menu",
                index = i
            })
        end
        table.insert(elements, {label = '<span style="color:red">Isprazni kosaru', value = "empty"})
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    pNotify("Sve je vraceno u crni market.", "error", 2500)
                end
                if data.current.value == "item_menu" then
                    ...
                    ESX.UI.Menu.Open(
                        ...
                        {
                            title    = "Kosara - " .. shopItem["label"] .. " - " .. shopItem["amount"] .. " komada",
                            align    = 'center',
                            elements = {
                                {label = shopItem["label"] .. " - $" .. shopItem["price"] * shopItem["amount"]},
                                {label = '<span style="color:red">Izbrisi item</span>', value = "deleteItem"},
                            },
                        },
                        function(data2, menu2)
                            if data2.current["value"] == "deleteItem" then
                                pNotify("Maknuo " .. Basket[index]["amount"] .." ".. Basket[index]["label"] .. " iz kosare.", "alert", 2500)
